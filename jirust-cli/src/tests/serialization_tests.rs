...
mod tests {
    ...
    fn test_printable_data_generic_serialization() {
        ...
        assert!(serialized.len() > 50, "Serialized data should be substantial");
    }
    ...
    fn test_printable_data_issue_created_serialization() {
        ...
        let printable = PrintableData::IssueCreated { issues: issues.clone() };
        ...
        assert!(serialized.len() > 100, "Serialized data should be substantial");
    }
    ...
    fn test_printable_data_transitioned_issue_serialization() {
        ...
        let printable = PrintableData::TransitionedIssue { issues: issues.clone() };
    ...
    fn test_config_file_toml_serialization() {
        ...
        assert_eq!(read_config.get_standard_resolution(), config.get_standard_resolution());
        assert_eq!(read_config.get_transition_name("start"), Some(vec!["In Progress".to_string()]));
        assert_eq!(read_config.get_transition_name("finish"), Some(vec!["Done".to_string()]));
    }
    ...
    fn test_config_file_round_trip_serialization() {
        ...
        for i in 0..5 {
            ...
            assert!(write_result.is_ok(), "Round trip {} write should succeed", i);
            ...
            assert_eq!(read_config.get_standard_resolution(), original_config.get_standard_resolution());
            assert_eq!(read_config.get_standard_resolution_comment(), original_config.get_standard_resolution_comment());
        }
    }
    ...
    fn test_complex_printable_data_serialization() {
        ...
        fields.insert("description".to_string(), json!("Very detailed description"));
        fields.insert("priority".to_string(), json!({"name": "High", "id": "2", "iconUrl": "icon.png"}));
        fields.insert("status".to_string(), json!({"name": "To Do", "category": {"key": "new"}}));
        fields.insert("assignee".to_string(), json!({"displayName": "Test User", "emailAddress": "test@example.com"}));
        fields.insert("components".to_string(), json!([{"name": "Frontend"}, {"name": "Backend"}]));
        fields.insert("labels".to_string(), json!(["urgent", "customer", "bug"]));
        fields.insert("customfield_10001".to_string(), json!({"value": "Custom Value"}));
        ...
        let printable = PrintableData::IssueData { issues: vec![complex_issue] };
        ...
        assert!(serialized.len() > 500, "Complex serialized data should be substantial");
    }
    ...
    fn test_serialization_with_unicode_and_special_chars() {
        ...
        assert!(serialized.len() > 100, "Unicode serialized data should be substantial");
    }
    ...
    fn test_serialization_edge_cases() {
        ...
        let serialized = serde_json::to_string(&printable).expect("Should serialize empty structures");
    ...
    fn test_large_data_serialization() {
        ...
        assert!(serialized.len() > 10000, "Serialized data should be substantial");
        
        // Verify large data serialization performance
        assert!(serialized.len() > 50000, "Large serialized data should be very substantial");
    ...
    fn test_config_serialization_with_complex_transitions() {
        ...
        config.add_transition_name("with-spaces".to_string(), "Transition With Spaces".to_string());
        config.add_transition_name("with_underscores".to_string(), "Transition_With_Underscores".to_string());
        config.add_transition_name("with.dots".to_string(), "Transition.With.Dots".to_string());
        config.add_transition_name("unicode测试".to_string(), "Unicode Transition 测试".to_string());
        ...
        assert_eq!(read_config.get_transition_name("simple"), Some(vec!["Simple Transition".to_string()]));
        assert_eq!(read_config.get_transition_name("with-spaces"), Some(vec!["Transition With Spaces".to_string()]));
        assert_eq!(read_config.get_transition_name("unicode测试"), Some(vec!["Unicode Transition 测试".to_string()]));
        assert_eq!(read_config.get_transition_name("emoji🚀"), Some(vec!["Emoji Transition 🚀".to_string()]));
        assert_eq!(read_config.get_transition_name("special!@#"), Some(vec!["Special Chars !@#".to_string()]));
        
        // Verify other fields
        assert_eq!(read_config.get_jira_url(), "https://complex-transitions.atlassian.net");
        assert!(read_config.get_standard_resolution_comment().contains("测试"));
