...
mod tests {
    ...
    #[test]
    fn test_set_cfg_auth_with_reader() {
        let temp_dir = tempdir().expect("Failed to create temp dir");
        let config_file_path = temp_dir.path().join("auth_config.toml");
        let runner = ConfigCmdRunner::new(config_file_path.to_string_lossy().to_string());

        let cfg = ConfigFile::default();
        let mut input = Cursor::new("test-user\n");
        let updated = runner
            .set_cfg_auth_with_reader(cfg, &mut input, || Ok("secret-token".to_string()))
            .expect("auth should be saved");

        let (user, token) = AuthData::from_base64(updated.get_auth_key());
        assert_eq!(user, "test-user");
        assert_eq!(token, "secret-token");
    }

    #[test]
    fn test_set_cfg_jira_with_reader() {
        let temp_dir = tempdir().expect("Failed to create temp dir");
        let config_file_path = temp_dir.path().join("jira_config.toml");
        let runner = ConfigCmdRunner::new(config_file_path.to_string_lossy().to_string());

        let cfg = ConfigFile::default();
        let mut input =
            Cursor::new("https://jira.example.com\n{\"name\":\"Done\"}\nResolved automatically\n");
        let updated = runner
            .set_cfg_jira_with_reader(cfg, &mut input)
            .expect("jira data should be stored");

        assert_eq!(updated.get_jira_url(), "https://jira.example.com");
        assert_eq!(updated.get_standard_resolution(), "{\"name\":\"Done\"}\n");
        assert_eq!(
            updated.get_standard_resolution_comment(),
            "Resolved automatically\n"
        );
    }
    ...
    fn test_config_cmd_runner_init_file_with_nested_directories() {
        ...
        assert!(
            result.is_ok(),
            "Failed to initialize config file with nested directories"
        );
    ...
    fn test_config_cmd_runner_show_cfg_with_complex_config() {
        ...
        transitions.insert(
            "start".to_string(),
            toml::Value::String("In Progress".to_string()),
        );
        transitions.insert(
            "finish".to_string(),
        ...
        transitions.insert(
            "close".to_string(),
            toml::Value::String("Closed".to_string()),
        );
    ...
    fn test_config_cmd_runner_multiple_instances() {
        ...
        assert!(
            result1.is_ok(),
            "First runner should initialize successfully"
        );
        assert!(
            result2.is_ok(),
            "Second runner should initialize successfully"
        );
        assert!(
            result3.is_ok(),
            "Third runner should initialize successfully"
        );

        // Verify all files were created
        assert!(
            Path::new(&config1_path).exists(),
            "First config file should exist"
        );
        assert!(
            Path::new(&config2_path).exists(),
            "Second config file should exist"
        );
        assert!(
            Path::new(&config3_path).exists(),
            "Third config file should exist"
        );
    }
    ...
    fn test_config_cmd_runner_with_unicode_paths() {
        ...
        assert!(
            Path::new(&unicode_path).exists(),
            "Unicode named file should exist"
        );
    }
    ...
    fn test_config_cmd_runner_with_spaces_in_path() {
        let temp_dir = tempdir().expect("Failed to create temp dir");
        let spaced_path = temp_dir
            .path()
            .join("config with spaces.toml")
            .to_string_lossy()
            .to_string();
