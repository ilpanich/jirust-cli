...
mod tests {
    ...
    fn test_printable_data_empty_collections() {
        ...
        let printable_projects = PrintableData::Project { projects: Vec::new() };
        let printable_versions = PrintableData::Version { versions: Vec::new() };
    ...
    fn test_printable_data_serialization() {
        ...
        let serialized = serde_json::to_string(&printable).expect("Failed to serialize PrintableData");
    ...
    fn test_output_args_creation() {
        ...
        assert!(matches!(output_args.output_format, Some(OutputValues::Json)));
