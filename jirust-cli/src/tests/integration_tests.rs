...
mod tests {
    ...
    use crate::args::commands::{OutputArgs, OutputValues, OutputTypes, PaginationArgs};
    use jira_v3_openapi::models::{Project, Version, CreatedIssue};
    ...
    fn test_printable_data_workflow() {
        ...
        let json_output = serde_json::to_string(&project_data).expect("Failed to serialize project data");
        ...
        let version_json = serde_json::to_string(&version_data).expect("Failed to serialize version data");
    ...
    fn test_configuration_with_various_transition_types() {
        ...
        config.add_transition_name("selected".to_string(), "Selected for Development".to_string());
    ...
    fn test_output_configuration_combinations() {
        ...
        assert!(matches!(json_summary.output_format, Some(OutputValues::Json)));
        ...
        assert!(matches!(table_summary.output_format, Some(OutputValues::Table)));
        assert!(matches!(table_summary.output_type, Some(OutputTypes::Basic)));
        
        assert!(matches!(table_full.output_format, Some(OutputValues::Table)));
    ...
    fn test_complex_printable_data_serialization() {
        ...
        let issue_data = PrintableData::IssueCreated { issues: created_issues };
        
        // Serialize and verify structure
        let serialized = serde_json::to_string(&issue_data).expect("Failed to serialize issue data");
        ...
        let generic_serialized = serde_json::to_string(&generic_printable).expect("Failed to serialize generic data");
    ...
    fn test_config_modification_workflow() {
        ...
        assert_eq!(config.get_standard_resolution_comment(), "Issue has been fixed");
