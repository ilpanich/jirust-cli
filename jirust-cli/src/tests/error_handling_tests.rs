...
mod tests {
    ...
    fn test_config_file_serialization_with_invalid_characters() {
        ...
        transitions.insert("weird\"key".to_string(), toml::Value::String("value with\nnewlines\tand\ttabs".to_string()));
        transitions.insert("unicode🚀".to_string(), toml::Value::String("unicode value 🎉".to_string()));
        ...
        assert!(result.is_ok(), "Should handle special characters in serialization");
        
        // Verify we can read it back
        let read_result = ConfigFile::read_from_file(config_path.to_str().unwrap());
        assert!(read_result.is_ok(), "Should be able to read back config with special characters");
    }
    ...
    fn test_printable_data_with_malformed_json() {
        ...
        let printable = PrintableData::Generic { data: problematic_data };
    ...
    fn test_transition_name_management_edge_cases() {
        ...
        assert_eq!(result, Some(vec!["First Value".to_string(), "Second Value".to_string()]));
    ...
    fn test_config_file_clone_and_equality() {
        ...
        assert_eq!(original.get_standard_resolution(), cloned.get_standard_resolution());
        assert_eq!(original.get_standard_resolution_comment(), cloned.get_standard_resolution_comment());
    ...
    fn test_memory_usage_with_large_configs() {
        ...
        for i in 0..100 {
            ...
            assert_eq!(transition_name, Some(vec![format!("Transition Name {}", i)]));
        }
        ...
        assert!(write_result.is_ok(), "Should handle large config serialization");
        
        // Test reading back large config
        let read_result = ConfigFile::read_from_file(large_config_path.to_str().unwrap());
        assert!(read_result.is_ok(), "Should handle large config deserialization");
    }
}