...
pub async fn get_project_role_details(configuration: &configuration::Configuration, project_id_or_key: &str, current_member: Option<bool>, exclude_connect_addons: Option<bool>, exclude_other_service_roles: Option<bool>) -> Result<Vec<models::ProjectRoleDetails>, Error<GetProjectRoleDetailsError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = exclude_other_service_roles {
        local_var_req_builder = local_var_req_builder.query(&[("excludeOtherServiceRoles", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
