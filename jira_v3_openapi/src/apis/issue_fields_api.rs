...
/// struct for typed errors of method [`get_project_fields`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetProjectFieldsError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_project_fields(configuration: &configuration::Configuration, project_id: Vec<i64>, work_type_id: Vec<i64>, start_at: Option<u64>, max_results: Option<u32>, field_id: Option<Vec<String>>) -> Result<models::PageBean2ProjectFieldBean, Error<GetProjectFieldsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/rest/api/3/projects/fields", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    local_var_req_builder = match "multi" {
        "multi" => local_var_req_builder.query(&work_type_id.into_iter().map(|p| ("workTypeId".to_owned(), p.to_string())).collect::<Vec<(std::string::String, std::string::String)>>()),
        _ => local_var_req_builder.query(&[("workTypeId", &work_type_id.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string())]),
    };
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetProjectFieldsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
