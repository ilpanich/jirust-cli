...
pub async fn find_groups(configuration: &configuration::Configuration, account_id: Option<&str>, query: Option<&str>, exclude: Option<Vec<String>>, exclude_id: Option<Vec<String>>, max_results: Option<i32>, case_insensitive: Option<bool>, user_name: Option<&str>, include_teams: Option<bool>) -> Result<models::FoundGroups, Error<FindGroupsError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = include_teams {
        local_var_req_builder = local_var_req_builder.query(&[("includeTeams", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
