/*
 * The Jira Cloud platform REST API
 *
 * Jira Cloud platform REST API documentation
 *
 * The version of the OpenAPI document: 1001.0.0-SNAPSHOT-af42c50d50804c2f1b8ad4bb80d52c53890867e2
 * Contact: ecosystem@atlassian.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct BulkWorklogKeyResponseBean {
    /// A list of successfully retrieved worklogs with their issue and worklog IDs.
    #[serde(rename = "worklogs", skip_serializing_if = "Option::is_none")]
    pub worklogs: Option<Vec<models::WorklogKeyResult>>,
}

impl BulkWorklogKeyResponseBean {
    pub fn new() -> BulkWorklogKeyResponseBean {
        BulkWorklogKeyResponseBean {
            worklogs: None,
        }
    }
}

