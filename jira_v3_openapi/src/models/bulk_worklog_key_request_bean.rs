/*
 * The Jira Cloud platform REST API
 *
 * Jira Cloud platform REST API documentation
 *
 * The version of the OpenAPI document: 1001.0.0-SNAPSHOT-af42c50d50804c2f1b8ad4bb80d52c53890867e2
 * Contact: ecosystem@atlassian.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct BulkWorklogKeyRequestBean {
    /// A list of issue and worklog ID pairs.
    #[serde(rename = "requests", skip_serializing_if = "Option::is_none")]
    pub requests: Option<Vec<models::WorklogCompositeKey>>,
}

impl BulkWorklogKeyRequestBean {
    pub fn new() -> BulkWorklogKeyRequestBean {
        BulkWorklogKeyRequestBean {
            requests: None,
        }
    }
}

