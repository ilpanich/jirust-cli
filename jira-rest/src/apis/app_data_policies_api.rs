...
/// struct for typed successes of method [`get_policies`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPoliciesSuccess {
    Status200(models::ProjectDataPolicies),
    UnknownValue(serde_json::Value),
}

/// struct for typed successes of method [`get_policy`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPolicySuccess {
    Status200(models::WorkspaceDataPolicy),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_policies(configuration: &configuration::Configuration, ids: Option<&str>) -> Result<ResponseContent<GetPoliciesSuccess>, Error<GetPoliciesError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetPoliciesSuccess> = serde_json::from_str(&local_var_content).ok();
...
pub async fn get_policy(configuration: &configuration::Configuration, ) -> Result<ResponseContent<GetPolicySuccess>, Error<GetPolicyError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetPolicySuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
