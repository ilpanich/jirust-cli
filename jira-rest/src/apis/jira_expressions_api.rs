...
/// struct for typed successes of method [`analyse_expression`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AnalyseExpressionSuccess {
    Status200(models::JiraExpressionsAnalysis),
    UnknownValue(serde_json::Value),
}

/// struct for typed successes of method [`evaluate_jira_expression`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum EvaluateJiraExpressionSuccess {
    Status200(models::JiraExpressionResult),
    UnknownValue(serde_json::Value),
}
...
pub async fn analyse_expression(configuration: &configuration::Configuration, jira_expression_for_analysis: models::JiraExpressionForAnalysis, check: Option<&str>) -> Result<ResponseContent<AnalyseExpressionSuccess>, Error<AnalyseExpressionError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<AnalyseExpressionSuccess> = serde_json::from_str(&local_var_content).ok();
...
pub async fn evaluate_jira_expression(configuration: &configuration::Configuration, jira_expression_eval_request_bean: models::JiraExpressionEvalRequestBean, expand: Option<&str>) -> Result<ResponseContent<EvaluateJiraExpressionSuccess>, Error<EvaluateJiraExpressionError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<EvaluateJiraExpressionSuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
