...
/// struct for typed successes of method [`get_issue_security_level`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetIssueSecurityLevelSuccess {
    Status200(models::SecurityLevel),
    UnknownValue(serde_json::Value),
}

/// struct for typed successes of method [`get_issue_security_level_members`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetIssueSecurityLevelMembersSuccess {
    Status200(models::PageBeanIssueSecurityLevelMember),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_issue_security_level(configuration: &configuration::Configuration, id: &str) -> Result<ResponseContent<GetIssueSecurityLevelSuccess>, Error<GetIssueSecurityLevelError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetIssueSecurityLevelSuccess> = serde_json::from_str(&local_var_content).ok();
...
pub async fn get_issue_security_level_members(configuration: &configuration::Configuration, issue_security_scheme_id: i64, start_at: Option<i64>, max_results: Option<i32>, issue_security_level_id: Option<Vec<String>>, expand: Option<&str>) -> Result<ResponseContent<GetIssueSecurityLevelMembersSuccess>, Error<GetIssueSecurityLevelMembersError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetIssueSecurityLevelMembersSuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
