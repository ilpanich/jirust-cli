...
/// struct for typed successes of method [`get_custom_field_configuration`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetCustomFieldConfigurationSuccess {
    Status200(models::PageBeanContextualConfiguration),
    UnknownValue(serde_json::Value),
}

/// struct for typed successes of method [`update_custom_field_configuration`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateCustomFieldConfigurationSuccess {
...
pub async fn get_custom_field_configuration(configuration: &configuration::Configuration, field_id_or_key: &str, id: Option<Vec<i64>>, field_context_id: Option<Vec<i64>>, issue_id: Option<i64>, project_key_or_id: Option<&str>, issue_type_id: Option<&str>, start_at: Option<i64>, max_results: Option<i32>) -> Result<ResponseContent<GetCustomFieldConfigurationSuccess>, Error<GetCustomFieldConfigurationError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetCustomFieldConfigurationSuccess> = serde_json::from_str(&local_var_content).ok();
...
pub async fn update_custom_field_configuration(configuration: &configuration::Configuration, field_id_or_key: &str, custom_field_configurations: models::CustomFieldConfigurations) -> Result<ResponseContent<UpdateCustomFieldConfigurationSuccess>, Error<UpdateCustomFieldConfigurationError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<UpdateCustomFieldConfigurationSuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
