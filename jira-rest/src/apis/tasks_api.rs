...
/// struct for typed successes of method [`cancel_task`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CancelTaskSuccess {
...
pub enum GetTaskSuccess {
    Status200(models::TaskProgressBeanObject),
    UnknownValue(serde_json::Value),
}
...
pub async fn cancel_task(configuration: &configuration::Configuration, task_id: &str) -> Result<ResponseContent<CancelTaskSuccess>, Error<CancelTaskError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<CancelTaskSuccess> = serde_json::from_str(&local_var_content).ok();
...
pub async fn get_task(configuration: &configuration::Configuration, task_id: &str) -> Result<ResponseContent<GetTaskSuccess>, Error<GetTaskError>> {
    let local_var_configuration = configuration;
