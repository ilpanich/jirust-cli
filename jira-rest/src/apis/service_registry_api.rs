...
/// struct for typed successes of method [`service_registry_resource_period_services_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ServiceRegistryResourcePeriodServicesGetSuccess {
    Status200(Vec<models::ServiceRegistry>),
...
pub async fn service_registry_resource_period_services_get(configuration: &configuration::Configuration, service_ids: Vec<String>) -> Result<ResponseContent<ServiceRegistryResourcePeriodServicesGetSuccess>, Error<ServiceRegistryResourcePeriodServicesGetError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<ServiceRegistryResourcePeriodServicesGetSuccess> = serde_json::from_str(&local_var_content).ok();
