...
pub enum GetStatusSuccess {
    Status200(models::StatusDetails),
    UnknownValue(serde_json::Value),
}

/// struct for typed successes of method [`get_statuses`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetStatusesSuccess {
    Status200(Vec<models::StatusDetails>),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_status(configuration: &configuration::Configuration, id_or_name: &str) -> Result<ResponseContent<GetStatusSuccess>, Error<GetStatusError>> {
...
pub async fn get_statuses(configuration: &configuration::Configuration, ) -> Result<ResponseContent<GetStatusesSuccess>, Error<GetStatusesError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetStatusesSuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
