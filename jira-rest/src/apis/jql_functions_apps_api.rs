...
/// struct for typed successes of method [`get_precomputations`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPrecomputationsSuccess {
    Status200(models::PageBeanJqlFunctionPrecomputationBean),
    UnknownValue(serde_json::Value),
}

/// struct for typed successes of method [`update_precomputations`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdatePrecomputationsSuccess {
...
pub async fn get_precomputations(configuration: &configuration::Configuration, function_key: Option<Vec<String>>, start_at: Option<i64>, max_results: Option<i32>, order_by: Option<&str>) -> Result<ResponseContent<GetPrecomputationsSuccess>, Error<GetPrecomputationsError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<GetPrecomputationsSuccess> = serde_json::from_str(&local_var_content).ok();
...
pub async fn update_precomputations(configuration: &configuration::Configuration, jql_function_precomputation_update_request_bean: models::JqlFunctionPrecomputationUpdateRequestBean) -> Result<ResponseContent<UpdatePrecomputationsSuccess>, Error<UpdatePrecomputationsError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<UpdatePrecomputationsSuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
